...
class User(Base):
    ...
    # Clerk user ID (e.g., "user_abc...")
    clerk_user_id = Column(String, unique=True, nullable=True)
    name = Column(String, nullable=False)
    email = Column(String, unique=True, nullable=False)
    password_hash = Column(Text, nullable=True)  # Nullable for Clerk users
    ...
    user_prompts = relationship(
        "UserPrompt", back_populates="user", cascade="all, delete-orphan")


class Quota(Base):
    __tablename__ = "quotas"
    user_id = Column(UUID(as_uuid=True), primary_key=True)
    daily_limit = Column(Integer, default=100000, nullable=False)
